...
|--------|-------|-------------|
| **NL pairs from query logs** | ~10,000 | Real ADS queries from search logs, paired with natural language descriptions generated by Claude Sonnet 4. Validated to prevent query syntax leaking into NL text. |
| **Gold examples** | 4,557 | Hand-curated NL-to-query pairs covering diverse query patterns (topics, authors, operators, date ranges, compound queries). Also used as the retrieval index for the deterministic pipeline. |
| **Synthetic pairs** | 2,526 | Programmatically generated from templates using seed lists of topics, astronomers, objects, journals, and institutions. Covers edge cases and common patterns. |

After combining (~17,000 total), the dataset is validated, deduplicated by both NL text and query text, and split into train/val sets with a fixed seed for reproducibility.
